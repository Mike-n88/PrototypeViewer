...
window.onload = function () {
    ...
    document.getElementById('filterLayerSelect').addEventListener('change', function () {
        (0, _formValues.getFeatureName)(document.getElementById('layerSelect').value);
    });

    //Eventlistener going off clicking "Filters" button, will show filters window
    document.getElementById('buttonFilterApply').addEventListener('click', function () {
        (0, _filter.filter)(document.getElementById('filterLayerSelect').value, document.getElementById('featureNameSelect').value, document.getElementById('operatorSelect1').value, document.getElementById('checkWaarde1').value);
    });
...
},{"./filter.js":3,"./formValues.js":4,"./map.js":5,"./window.js":7,"jQuery":8}],2:[function(require,module,exports){
'use strict';
...
function createFeatureLayer(serviceUrl, layer) {
    var vectorSource = new ol.source.Vector({
        loader: function loader(extent, resolution, projection) {
            var url = serviceUrl + '/' + layer + '/query/?f=json&' + 'returnGeometry=true&spatialRel=esriSpatialRelIntersects&geometry=' + encodeURIComponent('{"xmin":' + extent[0] + ',"ymin":' + extent[1] + ',"xmax":' + extent[2] + ',"ymax":' + extent[3] + ',"spatialReference":{"wkid":102100}}') + '&geometryType=esriGeometryEnvelope&inSR=102100&outFields=*' + '&outSR=102100';
            $.ajax({
                url: url,
                dataType: 'jsonp',
...
function changeFeature(feature) {
    ...
    for (var s in str) {
        if (_typeof(str[s]) === 'object' || str[s] === 'geometry') {} else {
...
function filter(layerId, featureName1, operatorName1, checkWaarde1) {

    var url = (0, _map.getUrl)();
...
function getFeatureName(numberLayer) {
  ...
  var checkWaarde1 = $('#checkWaarde1').find('option:0').remove().end();
  //let aantal;
...
},{"./map.js":5,"jQuery":8}],5:[function(require,module,exports){
(function (global){
'use strict';
...
function addLayersToMap() {
    if (dataSource.toLowerCase().indexOf('mapserver') >= 0) {
        ...
        for (var i = 0; i < layers.length; i++) {
            var aid = layers[i].id;
            mapserverLayersArray[i] = (0, _featureservice.createFeatureLayer)(dataSource, aid);
            map.addLayer(mapserverLayersArray[i]);
            mapserverLayersArray[i].setVisible(false);
        }
    }
...
function createLayer(layerId) {
    if (dataSource.toLowerCase().indexOf('mapserver') >= 0) {
        return (0, _mapservice.createMapserverLayer)(layerId, dataSource);
    } else if (dataSource.toLowerCase().indexOf('featureserver') >= 0) {
...
function onMouseClick(browserEvent) {
    ...
    map.forEachFeatureAtPixel(pixel, function (feature) {
        ...
        document.getElementById('saveButton').addEventListener('click', function () {
            (0, _featureservice.changeFeature)(feature);
        });

        var str = feature.getProperties();
        for (var s in str) {
            if (_typeof(str[s]) === 'object') {} else {
...
},{"./featureservice.js":2,"./mapservice.js":6,"jQuery":8}],6:[function(require,module,exports){
'use strict';
